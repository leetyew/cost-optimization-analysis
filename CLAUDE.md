...
| **There are TWO prose shapes, and the second omits the `Q<n>.` echo entirely** | **confirmed 2026-08-04, and it explains the 1,047 "prose-unreadable" runs.** Shape A is `Q<n>. …\nA<n>. …\nEvidence. …` separated by a BLANK line. Shape B is bare, single-newline-separated answer lines with no question echo: `A1. 5\nA2. NULL\nA3. 3`. `ANSWER_BLOCK_RE` requires the echo, so shape B matched ZERO blocks and every answer fell back to `answer_dict`. **The shape varies BY RUN, not by record** — run_0 and run_1 can be shape A while run_2 is shape B, which is why an inspected record looks fine. `BARE_ANSWER_RE` now handles it, tried only as a fallback so shape-A runs parse byte-identically |
| Whether shape B ever carries an `Evidence` line | **unconfirmed — being measured.** The operator's sample showed consecutive `A<n>.` lines with none. This decides whether shape-B answers may enter the default-3 denominator: if the missing Evidence line is *structural to the format* rather than a finding about the search, scoring those 3s as default-3 would inflate the headline with a formatting artifact. Until the counters answer it, shape-B rows carry `parsed_from = 'answers_text_bare'`, which `_EVIDENCE_OBSERVABLE` deliberately does NOT match — so they stay out of the denominator exactly as they are today, and the headline does not move |
